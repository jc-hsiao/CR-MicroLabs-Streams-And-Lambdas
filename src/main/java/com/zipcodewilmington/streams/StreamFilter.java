...
public class StreamFilter {
    ...
    public List<Person> toListOneLine() {
        return personStream.filter(p -> p.getName().substring(0, 1).equals(startingCharacter)).collect(Collectors.toList());
    }
    ...
    public Person[] toArrayOneLine() {
        return personStream.filter(p -> p.getName().substring(0, 1).equals(startingCharacter)).toArray(Person[]::new);
    }
